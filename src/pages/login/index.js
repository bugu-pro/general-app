  ...
  render() {
    ...
    return (
      <Form layout="horizontal" wrapperCol={{span: 18, offset: 3}} name="control-ref" ref={this.formRef}>
        ...
        >
          {({getFieldValue}) => {
            return getFieldValue('userName') !== 'other' ? (
              <Form.Item name="userName" label="" rules={[{required: true, message: '请输入帐号'}]}>
                <Input
        ...
        <Form.Item noStyle
          shouldUpdate={(prevValues, currentValues) => prevValues.pswd !== currentValues.pswd}
        >
          {({getFieldValue}) => {
            return getFieldValue('pswd') !== 'other' ? (
              <Form.Item name="pswd" label="" rules={[{required: true, message: '请输入密码'}]}>
                <Input.Password
